    ...
    private function processBuffer(string $buffer, $templateName): string
    {
        if (TL_MODE == 'FE' && 'fe_page' === $templateName) 
        {
            ...
            if ($rootPage->cta_position) {
                if ($rootPage->cta_1_link) { $html_strukutur .= generateHTML($rootPage->cta_1, $rootPage->cta_1_text, PageModel::findById($rootPage->cta_1_link)); }
                if ($rootPage->cta_2_link) { $html_strukutur .= generateHTML($rootPage->cta_2, $rootPage->cta_2_text, PageModel::findById($rootPage->cta_2_link)); }
                if ($rootPage->cta_3_link) { $html_strukutur .= generateHTML($rootPage->cta_3, $rootPage->cta_3_text, PageModel::findById($rootPage->cta_3_link)); }
                if ($rootPage->cta_4_link) { $html_strukutur .= generateHTML($rootPage->cta_4, $rootPage->cta_4_text, PageModel::findById($rootPage->cta_4_link)); }
            }
